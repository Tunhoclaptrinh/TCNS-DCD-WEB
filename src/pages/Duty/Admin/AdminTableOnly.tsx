                    ...
                    <tr key={row.key} style={{ background: rIdx % 2 === 0 ? '#fdf2f8' : '#e0f2fe' }}>
                      ...
                      {weekDays.map((day, dIdx) => {
                        ...
                        const daySlots = slots.filter(s => {
                           ...
                           const rowFullName = `${row.name} - ${row.shortName}`.toLowerCase();
                                   ...
                                   slot.assignedUsers.map((u: any, uIdx: number) => (
                                     <Tooltip key={u.id} title={u.name}>
                                       <div 
                                         ...
                                         style={{ 
                                           padding: '4px 8px', 
                                           borderBottom: uIdx < (slot.assignedUsers?.length || 0) - 1 ? '1px dashed #cbd5e1' : 'none',
                                           fontSize: '12px',
                                           color: '#1e293b',
                                         ...
                                         onMouseEnter={(e) => {
                                           if (isAdmin) e.currentTarget.style.backgroundColor = '#f1f5f9';
                                         }}
                                         onMouseLeave={(e) => {
                                         ...
                                         onClick={(e) => {
                                           if (!isAdmin) return;
                                           e.stopPropagation();
                                           window.open(`/users/detail/${u.id}`, '_blank');
                                         }}
                                       >
                                         <Typography.Text ellipsis style={{ width: '100%', fontSize: 'inherit', color: 'inherit' }}>
                                           {u.name}
                                         </Typography.Text>
                                       </div>
                                     </Tooltip>
